...
from langfuse_agents.observability.tracing import TOOL, traced, update_span
...
class ToolRegistry:
    ...
    @traced(name="tool.execute", as_type=TOOL)
    def execute(self, name: str, arguments: dict[str, Any]) -> ToolExecution:
        ...
        update_span(name=f"tool:{name}", metadata={"risk": tool.risk,
