...
|---|---|---|---|
| LLM01 Prompt injection (direct) | User overrides instructions | Pattern-based input guardrail; blocked **before** the LLM; outcome annotated on the span | `security/guardrails.py` |
...
| LLM02 Sensitive information disclosure | PII in prompts, traces, logs, answers | Redaction at input, at payload serialisation (span attributes and retrieved document content), in logs and on output | `security/pii.py`, `observability/tracing.py`, `logging_config.py` |
...
| LLM10 Unbounded consumption | Loops, huge inputs, cost blow-ups | Step budget, input length limit, token/cost tracking, retry cap, OTel head sampling | `config.py`, `core/agent.py` |
...
| Observability data exposure | Traces contain business content | Self-hostable Phoenix (data stays in your network), project separation, retention settings, redaction on by default; OTLP lets you keep telemetry inside an existing collector | `docs/OBSERVABILITY.md` |
...
- [ ] Self-host Phoenix inside the same trust boundary for confidential data; restrict UI access
      and configure retention; secure the collector endpoint (auth header / network policy).
