...
* Full Opik trace: agent -> guardrails -> llm spans -> tool spans, plus online feedback scores.
"""
...
from enterprise_agents.core.llm import LLMClient, ToolCall
from enterprise_agents.core.tools import ToolRegistry
from enterprise_agents.observability import tracing
from enterprise_agents.observability.usage import UsageTracker
from enterprise_agents.security.approval import ApprovalHandler, deny_all
from enterprise_agents.security.guardrails import (
    REFUSAL_MESSAGE,
    InputGuardrail,
    OutputGuardrail,
    wrap_untrusted,
...
class ToolCallingAgent:
    ...
    @tracing.traced(name="agent.run", span_type="general")
    def run(self, user_input: str, session_id: str | None = None,
        ...
        tracing.update_trace(
            ...
            metadata={"agent_version": self.config.version, "model": self.llm.model,
                      "environment": self.environment, "user_id": user_id},
        ...
        try:
            ...
            if not guard.allowed:
                ...
                tracing.log_trace_feedback("input_guardrail_pass", 0.0, ";".join(guard.reasons))
                logger.warning("Input blocked", extra={"agent": self.config.name,
                                                       "reasons": guard.reasons})
                return self._finish(result, usage, start)
            tracing.log_trace_feedback("input_guardrail_pass", 1.0)
            ...
            for step in range(1, self.config.max_steps + 1):
                ...
                if not response.tool_calls:
                    ...
                    tracing.log_trace_feedback("output_guardrail_pass", 1.0 if out.allowed else 0.0,
            ...
            tracing.log_trace_feedback("completed_within_step_budget", 0.0)
            return self._finish(result, usage, start)
